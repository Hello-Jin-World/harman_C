...
#include <string.h>
#include <stdlib.h>
enum { RESERVATION = 1, CHECKOUT, MODIFICATION, ONE_INQUIRE,
ALL_INQUIRE, EXIT = 9 };
typedef struct
_
node {
...
char address[40]; // 주소
int room_price; // 요금
char enter_date[20]; // 입실 시각char exit_date[20]; // 퇴실 시각
printf("호텔 객실 예약\n");
...
room_price = 500000;strcpy(new_reservation->enter_date, "오후 3시");
strcpy(new_reservation->exit_date, "오전 11시");
}
{
...
t_NODE* current = *head;
while (current != NULL) {
if (strcmp(current->name, now_name) == 0 && strcmp(current->phone,
now_phone) == 0) {
printf("^^^^^^ 예약 변경 ^^^^^^\n");
printf("1. 예약자 성함 수정\n");
...
break;
case 3:
printf("새로운 주소 입력 : ");scanf("%s", new_address);
...
else if (new_roomno >= 2001 && new_roomno <
2910)
{
new_price = 500000;
...
else if (new_roomno >= 3001 && new_roomno <
3010)
{
new_price = 1000000;
strcpy(current->enter_date, "오후 3시");
...
t_NODE* current = *head;
while (current != NULL) {
if (strcmp(current->name, inq_name) == 0 && strcmp(current->phone,
inq_phone) == 0) {
printf("객실 번호 : %d\n", current->roomno);
printf("이름 : %s\n", current->name);
printf("전화번호 : %s\n", current->phone);
...
while (1) {
t_NODE* new_reservation = (t_NODE*)malloc(sizeof(t_NODE)); // 메모
리 동적 할당
